...
//modified:Eugene Linkov and Kartik Kulkarni
...
extern double gpu_encrypt_string(uchar* in, int length, uchar* key,  uchar* out, uchar * expkey);
extern double gpu_decrypt_string(uchar* in, int length, uchar* key, uchar* out, uchar * expkey);
extern void gpu_init();
void encrypt (uchar *mykey, char *name);
void decrypt (uchar *mykey, char *name);
...
int cpu_encrypt (uchar * input, int length, uchar * output )
{
...
int ch, idx = 0;
	...
	while( ch = (int)input [len], len != length ) {
		in[idx++] = ch;
		len++;
		if( idx % 16 )
			continue;
	...
	//printf ("\n after encryption from CPU file size is %d", len);
...
int cpu_decrypt (uchar * input, int length, uchar * output )
{
...
int gpu_encrypt (uchar * input, int length, uchar * output )
{

	uchar expkey[4 * Nb * (Nr + 1)];
	ExpandKey (samplekey, expkey);

	gpu_encrypt_string( input , length, samplekey , output , expkey);
...
int gpu_decrypt (uchar * input, int length, uchar * output )
{

	uchar expkey[4 * Nb * (Nr + 1)];
	ExpandKey (samplekey, expkey);

	gpu_decrypt_string( input , length, samplekey , output , expkey);
...
void sample (char * filename, int len)
{

	//Rounding the length to 16
	len = len - (len % 16);

	FILE *fp, *fpE, *fpD;
	fp=fopen(filename, "rb");
	fpE=fopen("encr.txt", "wb");
	fpD=fopen("decr.txt", "wb");

	unsigned char * textip = (unsigned char *)malloc (4096000);
	unsigned char * textop = (unsigned char *)malloc (4096000);

	unsigned char * textip1 = (unsigned char *)malloc (4096000);
	unsigned char * textop1 = (unsigned char *)malloc (4096000);
	...
	ExpandKey (samplekey, expkey);
	//Encrypt (textip, expkey, textop);

	//rd_clock(&start);
        start=gettime();
	//Encrypt (textip, expkey,textop);
	encrypt (samplekey,filename);
	...
	printf ("\nencrypt time: %f, %f cycles per byte\n", diff, diff/1539989);
	

	start=gettime();
	//Decrypt (textop, expkey, textip);
	decrypt (samplekey,filename);
	...
	printf ("\ndecrypt time: %f, %f cycles per byte\n", diff, diff/1539989);

	
	//GPU encrypt test
        printf ("\nNow lets try GPU encryption, and the op is: \n");

	FILE *fpTemp;
	//rd_clock(&start);
        fpTemp=fopen(filename, "rb");
	start=gettime();
		
		len = fread (textip, sizeof(unsigned char), len,fpTemp);
		gpu_encrypt_string( textip , len, samplekey , textop1 , expkey);

		//Writing encrypted and decrypted data to the files	
		fwrite (textop1, sizeof(unsigned char), len, fpE);
	...
	printf ("\nGPU encrypt time: %f, %f cycles per byte\n", diff, diff/len);

        fclose (fpTemp);

	printf ("\nNow lets try GPU decryption, and the op is: \n");
	//rd_clock(&start);
	start=gettime();
		gpu_decrypt_string(textop1, len, samplekey, textip1, expkey);
		//Writing encrypted and decrypted data to the files	
		fwrite (textip1, sizeof(unsigned char), len, fpD);
	...
	printf ("\nGPU decrypt time: %f, %f cycles per byte\n", diff, diff/len);
...
double start,end, diff;
	...
	diff = end - start;
	printf ("encrypt time: %f, %f cycles per byte\n", diff, diff/16);
	...
	diff = end - start;
	printf ("decrypt time: %f, %f cycles per byte\n", diff, diff/16);
...
extern int __cdecl _setmode (int, int);
	...
	case 'e': encrypt(reinterpret_cast<uchar *>( argv[2] ), argv[3]); break;
	case 'd': decrypt(reinterpret_cast<uchar *> (argv[2] ), argv[3]); break;
	case 'b': bernstein(argv[2]);	break;
	case 's': sample(argv[2], atoi (argv[3])); break;
	case 't': tables(); break;
	}
}
